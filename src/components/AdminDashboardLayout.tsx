...
export function AdminDashboardLayout() {
  ...
  const menuItems = [
    ...
    { path: "/admin/dashboard/contadores", icon: Users, label: "Gestão de Escritórios" },
    { path: "/admin/dashboard/configuracoes", icon: Settings, label: "Configurações" },
  ];

  return (
    <div className="flex h-screen bg-gray-50 font-sans">
      {/* Sidebar Branca Padrão AgroPops */}
      <aside className="w-64 bg-white border-r border-gray-100 flex flex-col transition-all duration-300 relative z-20">
        <div className="h-20 flex items-center px-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-gray-800">AgroPops</h1>
              <p className="text-[10px] uppercase tracking-widest text-agro-secondary font-bold">Admin</p>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative z-10">
        <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-8 shadow-sm">
          <h2 className="text-xl font-bold text-gray-800">Painel de Administração</h2>
          <div className="flex items-center gap-3">
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-gray-50/50">
          {/* As telas filhas (AdminContadores, VisaoGeralSaaS) aparecerão aqui */}
          <Outlet />
        </div>
      </main>
    </div>

...
export function NotasFiscais() {
  ...
  // Recebendo parâmetros do Dashboard
  ...
  // NOVO: Controle de Dedutibilidade (Abate Imposto vs Uso Pessoal)
  ...
  const obterParametrosDeData = (filtro: string) => {
    ...
    if (filtro === "Personalizado" && dataInicio && dataFim) {
      return `?inicio=${dataInicio}&fim=${dataFim}`;
    }
    return "";
  };

  const chaveCacheNotas = (produtorId: string, periodo: string) =>
    `@AgroPops:notasCache:${produtorId}:${periodo}${dataInicio}${dataFim}`;
  ...
  }, [currentProducer, activePeriod, dataInicio, dataFim]);
  ...
  useEffect(() => {
    ...
    buscarNotas();
  }, [currentProducer?.id, activePeriod, dataInicio, dataFim]);
  ...
  // AGORA O FILTRO CONTEMPLA AS "SUBLUPAS" DE DEDUTIBILIDADE NAS SAÍDAS
  const notasFiltradas = notas.filter((nota) => {
    ...
    if (activeTab === "saida" && filtroDedutibilidade !== "todos") {
      // Se a pessoa quer dedutível, a nota TEM de ter pelo menos 1 item dedutível.
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-4">
        <div className="flex flex-col md:flex-row gap-4 items-center w-full">
          ...
          <div className="flex items-center gap-2 bg-gray-50 p-1 rounded-xl border border-gray-200 w-full md:w-auto">
            <button
              ...
              className={`px-4 py-1.5 text-sm font-medium rounded-lg ${activeTab === "todas" ? "bg-white text-gray-800 shadow-sm" : "text-gray-500"}`}
            >
              Todas
            </button>
            <button
              onClick={() => setActiveTab("entrada")}
              className={`px-4 py-1.5 text-sm font-medium rounded-lg ${activeTab === "entrada" ? "bg-white text-emerald-600 shadow-sm" : "text-gray-500"}`}
            >
              Entradas
            </button>
            <button
              onClick={() => setActiveTab("saida")}
              className={`px-4 py-1.5 text-sm font-medium rounded-lg ${activeTab === "saida" ? "bg-white text-rose-600 shadow-sm" : "text-gray-500"}`}
            >
              Saídas
            </button>
          </div>
        ...
        {/* SUB-FILTRO: Aparece apenas se estiver na aba Saídas */}
        {activeTab === "saida" && (
              ...
              {notasFiltradas.map((nota) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right">
                    <span
                      className={`font-bold ${nota.tipo === "ENTRADA" ? "text-emerald-600" : "text-gray-800"}`}
                    >
                      {nota.tipo === "ENTRADA" ? "+" : "-"}{" "}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-gray-600">
                Selecione o período das notas que deseja apagar.
              </p>
              <select
                value={deletePeriod}
